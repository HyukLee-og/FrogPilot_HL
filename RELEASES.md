패치 업데이트 (2026-04-08, offroad wake 수정 / 오프로드 콘솔 보정 / NNFF 문구 정리)
=========================================================================================
* GM offroad wake
  * `CarMake` params가 bytes로 저장되는 점을 반영해, GM 판별을 decode 기반으로 수정
  * 이 수정으로 GM 차량에서 `offroad_wake_watcher`가 실제로 문/도어핸들 CAN 신호를 감시할 수 있도록 정리
* 오프로드 웹 콘솔
  * offroad 콘솔의 `driver` 스냅샷은 더 이상 `RecordFront`에 의존하지 않고 대시보드 전용 캡처 경로로 직접 획득
  * `carState`가 없는 상황에서 `정차 / 파킹 / 문 / 벨트`가 실제 상태처럼 보이지 않도록 `확인 불가`로 표시
* startup / NNFF 알럿
  * `customStartupAlert`를 FrogPilot 전용 스타일이 아니라 일반 `normal` 스타일로 정리
  * NNFF 로드/미지원 알럿을 한국어 문구로 정리
* 문서
  * `README.md`에 `testing-v1-apn` 브랜치 전용 기능 요약 섹션 추가
