...
FrogPilotDevicePanel::FrogPilotDevicePanel(FrogPilotSettingsWindow *parent, bool forceOpen) : FrogPilotListWidget(parent), parent(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> deviceToggles {
    ...
    {"DisableForcedPowerLogic", tr("강제 전원 로직 비활성화"), tr("<b>Turn off FrogPilot's automatic power-down rules entirely.</b> When enabled, the device will stay powered on regardless of shutdown timer or low-voltage settings."), ""},
